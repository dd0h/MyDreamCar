...
class MapGenerator:
    ...
    def generate(self, start_point):
        ...
        while current_segment <= self.map_length - SAFE_EXCESS:
            ...
            for j in range(current_segment, current_segment + howMuchSegmentsToSide):
                if current_segment + howMuchSegmentsToSide <= self.map_length:
                    while not (MAX_LEFT_DEVIATION_OF_ROAD  < self.map_points[j] + curvature < MAX_RIGHT_DEVIATION_OF_ROAD ):
    ...
    def getNextMapPoint(self):
        if self.current_map_point > self.map_length - SAFE_EXCESS:
